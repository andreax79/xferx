PS/8
====

Directory
---------

PIP - PS/8 8K Programming System User's Guide, Pag 62

```
/L      List directory
/E      List directory in extended format (including empty areas)
/F      List directory in short format (file names only)
```

Example:
```
.R PIP
*/L


ABSLDR.SV   5 10/31/70
LIB8  .RL  30 10/30/70
PIP   .SV   9 11/2/70
EDIT  .SV   9 11/5/70
PAL8  .SV  14 11/5/70
FORT  .SV  25 11/1/70
SABR  .SV  23 11/5/70
LOADER.SV  11 11/2/70
CONVRT.SV   8 10/29/70
CONFIG.PA 119 11/6/70
DTC8  .SV   5
PFOCAL.SV  30
ESCAPE.SV   2
PS8   .BN  19
TOG8  .SV   7 11/26/72
 365 FREE BLOCKS

*
```

References
----------

* [PS/8 8K Programming System User's Guide](https://bitsavers.org/pdf/dec/pdp8/software/DEC-08-MEFA-D_8k_Programming_System_Users_Guide.pdf)
* [PS/8 Software Support Manual](https://bitsavers.org/pdf/dec/pdp8/ps8/DEC-08-MEXB-D_PS8_SWSupMan.pdf])

